import json, os, time, threading, hmac, hashlib, urllib.parse, math
...
class Binance:
    ...
    def __init__(self, key, secret, testnet=True):
        ...
        self._filters = {}   # symbol -> {"stepSize": float, "tickSize": float}
    ...
    def post(self, path, params):
        ...
        params["recvWindow"] = 5000
        url = f"{self.base}{path}?{self._sign(params)}"
        r = requests.post(url, headers=self._h(), timeout=10)
        r.raise_for_status()
        return r.json()
    ...
    def set_leverage(self, symbol, lev):
        return self.post("/fapi/v1/leverage", {"symbol": symbol, "leverage": lev})
    ...
    def stop_order(self, symbol, side, stop_price, qty):
        return self.post("/fapi/v1/order", {
            ...
            "stopPrice": round(stop_price, 8), "quantity": qty,
    ...
    def filters(self, symbol):
        """Return {stepSize, tickSize} for a symbol, cached."""
        ...
        for s in info["symbols"]:
            if s["symbol"] == symbol:
                ...
                self._filters[symbol] = {"stepSize": step, "tickSize": tick}
                return self._filters[symbol]
        self._filters[symbol] = {"stepSize": 0.001, "tickSize": 0.01}
        return self._filters[symbol]

    def top_symbols(self, n=20):
        """Top N USDT perp symbols by 24h quote volume."""
        d = self.get("/fapi/v1/ticker/24hr")
        bad = ("UP", "DOWN", "BULL", "BEAR")
        out = []
        for t in d:
            s = t["symbol"]
            if not s.endswith("USDT"): continue
            if any(b in s[:-4] for b in bad): continue
            out.append((s, float(t["quoteVolume"])))
...
def sig_smc(bars):
    ...
    if c > lh:
        risk = c - ll
        if risk > 0: return {"side": "long", "strategy": "SMC", "strength": 0.9,
                             "stop": ll, "target": c + 2*risk}
    if c < ll:
        risk = lh - c
        if risk > 0: return {"side": "short", "strategy": "SMC", "strength": 0.9,
                             "stop": lh, "target": c - 2*risk}
    return None
...
def sig_meanrev(bars, n=40, z=2.0):
    ...
    zz = (bars[-1]["c"] - m) / sd
    if zz <= -z: return {"side": "long", "strategy": "MEANREV",
                         "strength": min(1.0, abs(zz)/3), "stop": m - 2*sd, "target": m}
    if zz >= z:  return {"side": "short", "strategy": "MEANREV",
                         "strength": min(1.0, abs(zz)/3), "stop": m + 2*sd, "target": m}
    return None
...
def sig_reversal(bars):
    ...
    if r < 30 and (b["c"] - b["l"])/rng > 0.6:
        return {"side": "long", "strategy": "REVERSAL", "strength": (30-r)/30,
                "stop": b["l"], "target": b["c"] + 2*(b["c"]-b["l"])}
    if r > 70 and (b["c"] - b["l"])/rng < 0.4:
        return {"side": "short", "strategy": "REVERSAL", "strength": (r-70)/30,
                "stop": b["h"], "target": b["c"] - 2*(b["h"]-b["c"])}
    return None
...
def ensemble(sigs, cfg):
    ...
    return {"side": side, "stop": stop, "score": abs(score),
            "members": [s["strategy"] for s in members]}
...
class Risk:
    ...
    def update(self, eq):
        ...
        if d != self.day:
            self.day = d; self.day_start = eq; self.halted = False; self.reason = ""
...
class Bot:
    def __init__(self):
        ...
        self.client = None
        self.last_error = ""
        self.state = {"equity": 0, "position": None, "loops": 0,
                      "last_sig": None, "scanning": None, "debug": []}
        # track last known open position so we can log closes
        self._last_pos = None   # {"symbol", "side", "qty", "entry"}
    ...
    def start(self):
        ...
        if not self.cfg["api_key"]:
            self.last_error = "No API key. Paste it in Settings → SAVE → then START."
            self._say(self.last_error); return
        if not self.cfg["api_secret"]:
            self.last_error = "No API secret. Paste it in Settings → SAVE → then START."
            self._say(self.last_error); return

        try:
            self.client = Binance(self.cfg["api_key"], self.cfg["api_secret"],
                                  self.cfg["testnet"])
            bal = self.client.balance()
        except Exception as e:
            self.last_error = f"Binance auth failed: {e}"
            self._say(self.last_error); return

        self.risk = Risk(self.cfg, bal)
        self.risk.eq = bal; self.risk.peak = bal; self.risk.day_start = bal
        self.running = True
        self.thread = threading.Thread(target=self._loop, daemon=True)
        self.thread.start()
        self._say(f"started · {self.cfg['symbol']} {self.cfg['timeframe']} · "
                  f"{'TESTNET' if self.cfg['testnet'] else 'LIVE'} · "
                  f"balance ${bal:.2f}")
    ...
    def _any_position(self, symbols):
        for sym in symbols:
            try:
                p = self.client.position(sym)
                if p:
                    p["symbol"] = sym
                    return p
            except Exception:
    ...
    def _log_close_if_any(self, held):
        """Compare current position to last seen; log close if it vanished."""
        prev = self._last_pos
        if prev and held is None:
            try:
                exit_px = self.client.price(prev["symbol"])
            except Exception:
                exit_px = prev["entry"]
            direction = 1 if prev["side"] == "long" else -1
            pnl = (exit_px - prev["entry"]) * prev["qty"] * direction
            jlog(event="close", symbol=prev["symbol"], side=prev["side"],
                 qty=prev["qty"], entry=prev["entry"], exit=exit_px,
                 pnl=round(pnl, 4))
            self._say(f"CLOSE {prev['side']} {prev['symbol']} {prev['qty']} "
                      f"@ {exit_px:.4f}  pnl≈{pnl:+.4f}")
            self._last_pos = None
        elif held is not None:
            self._last_pos = {"symbol": held["symbol"], "side": held["side"],
                              "qty": held["qty"], "entry": held["entry"]}

    def _loop(self):
        cfg = self.cfg
        tf = cfg["timeframe"]
        scan_n = int(cfg.get("scan_top_n", 20))
        auto = bool(cfg.get("auto_scan", True))

        # set leverage on the fixed symbol if not scanning
        if not auto:
            try:
                self.client.set_leverage(cfg["symbol"], cfg["leverage"])
                self._say(f"leverage {cfg['leverage']}x set on {cfg['symbol']}")
            except Exception as e:
                self._say(f"leverage error: {e}")

        while self.running:
            try:
                # 1) which symbols this cycle
                if auto:
                    try:
                        symbols = self.client.top_symbols(scan_n)
                    except Exception as e:
                ...
                # 2) check if we already have a position anywhere
                held = self._any_position(symbols) if auto else self._any_position([cfg["symbol"]])

                # 3) score every symbol, log debug list
                scores = []   # (symbol, side, score)
                best = None
                for sym in symbols:
                    try:
                        bars = self.client.klines(sym, tf, 200)
                    except Exception:
                        continue
                    sigs = []
                    ...
                    if combo:
                        scores.append((sym, combo["side"], round(combo["score"], 3)))
                        if best is None or combo["score"] > best["combo"]["score"]:
                            best = {"sym": sym, "combo": combo, "bars": bars}

                scores.sort(key=lambda x: x[2], reverse=True)
                top_debug = [f"{s} {sd[0]} {sd[2]}" for s, sd in
                             [(x[0], (x[1], x[2])) for x in scores][:5]]
                self.state["debug"] = top_debug

                # 4) equity + risk
                eq = self.client.balance() + (held["pnl"] if held else 0)
                self.risk.update(eq)

                # 5) log close if position disappeared
                self._log_close_if_any(held)

                with LOCK:
                    self.state.update({
                        "equity": eq,
                        "position": held,
                        "loops": self.state["loops"] + 1,
                        "last_sig": best["combo"]["side"] if best else None,
                        "scanning": best["sym"] if best else None,
                    })

                if scores:
                    self._say(f"eq={eq:.2f} pos={'-' if not held else held['symbol'] + ':' + held['side']}"
                              f" | top: {', '.join(top_debug)}")
                else:
                    self._say(f"eq={eq:.2f} pos={'-' if not held else held['symbol'] + ':' + held['side']}"
                              f" | no signals from {len(symbols)} symbols")

                # 6) enter if flat and best signal beats threshold
                if held is None and best and not self.risk.halted:
                    sym   = best["sym"]
                    combo = best["combo"]
                    entry = best["bars"][-1]["c"]
                    stop  = combo["stop"]
                    ...
                    raw_qty = self.risk.size(entry, stop)
                    qty = round_step(raw_qty, f["stepSize"])

                    if qty <= 0:
                        self._say(f"skip {sym}: qty too small (raw={raw_qty:.6f})")
                    else:
                        ...
                        try:
                            ...
                            self.client.stop_order(
                                sym,
                                "SELL" if combo["side"] == "long" else "BUY",
                                stop, qty)
                            self._say(f"OPEN {combo['side']} {sym} {qty} @ {entry:.4f} "
                                      f"stop={stop:.4f} score={combo['score']:.2f}")
                            jlog(event="open", symbol=sym, side=combo["side"], qty=qty,
                                 entry=entry, stop=stop, members=combo["members"],
                                 score=combo["score"])
                            # remember it so we can log the close later
                            self._last_pos = {"symbol": sym, "side": combo["side"],
                                              "qty": qty, "entry": entry}
                        except Exception as e:
                            self._say(f"order error on {sym}: {e}")
...
.debug{font-size:.72rem;color:#94a3b8;font-family:ui-monospace,monospace}</style></head><body>
...
<div class=row><span class=muted>Scanning now</span><span id=scanning class=debug>—</span></div>
...
<div class=card><h2>Top signals this cycle</h2><div id=debug class=debug>—</div></div>
...
<p class=muted style="font-size:.75rem;margin-top:8px">Get testnet keys at <b>testnet.binancefuture.com</b>. Never paste live keys on a shared machine.</p></div>

<div class=card><h2>Market scanning</h2>
<div class=switch><span>Auto-scan top symbols</span><input id=auto_scan type=checkbox checked></div>
<label>Scan top N symbols<input id=scan_top_n type=number min=5 max=50 value=20></label>
<label>Fallback symbol (used if scanning is off)<input id=symbol placeholder=BTCUSDT></label></div>
...
<label>Max drawdown %<input id=dd type=number step=0.5 min=1 max=50></label>
<label>Min signal score<input id=minscore type=number step=0.05 min=0 max=2></label>
<label>Poll seconds<input id=poll type=number min=3 max=300></label></div>
...
document.getElementById("scanning").textContent=s.scanning||"—";
...
document.getElementById("debug").textContent=(s.debug&&s.debug.length)?s.debug.join("\\n"):"—";
const p=s.position;
document.getElementById("position").innerHTML=!p?'<span class=muted>flat</span>':`<div class=row><span>${p.symbol||""} ${p.side.toUpperCase()}</span><span>qty ${p.qty}</span></div><div class=row><span class=muted>entry</span><span>${p.entry}</span></div><div class=row><span class=muted>unrealized</span><span class="${p.pnl>=0?'pos':'neg'}">${p.pnl.toFixed(2)}</span></div>`;
document.getElementById("log").textContent=(s.log||[]).join("\\n")||"waiting…";
const c=s.cfg;
if(c && !cfgFilled){
  document.getElementById("symbol").value=c.symbol||"";
  document.getElementById("timeframe").value=c.timeframe||"15m";
  document.getElementById("leverage").value=c.leverage||3;
  document.getElementById("risk").value=((c.risk_per_trade_pct||0.005)*100).toFixed(2);
  document.getElementById("daily").value=((c.max_daily_loss_pct||0.02)*100).toFixed(2);
  document.getElementById("dd").value=((c.max_drawdown_pct||0.10)*100).toFixed(2);
  document.getElementById("minscore").value=c.min_signal_score||0.20;
  document.getElementById("poll").value=c.poll_seconds||15;
  document.getElementById("testnet").checked=c.testnet;
  document.getElementById("use_smc").checked=c.use_smc;
  document.getElementById("use_meanrev").checked=c.use_meanrev;
  document.getElementById("use_reversal").checked=c.use_reversal;
  document.getElementById("auto_scan").checked=c.auto_scan;
  document.getElementById("scan_top_n").value=c.scan_top_n||20;
  cfgFilled=true;
}
...
async function save(){
  const b={
    ...
    risk_per_trade_pct:parseFloat(document.getElementById("risk").value)/100,
    max_daily_loss_pct:parseFloat(document.getElementById("daily").value)/100,
    max_drawdown_pct:parseFloat(document.getElementById("dd").value)/100,
    min_signal_score:parseFloat(document.getElementById("minscore").value),
    poll_seconds:parseInt(document.getElementById("poll").value,10),
    testnet:document.getElementById("testnet").checked,
    use_smc:document.getElementById("use_smc").checked,
    use_meanrev:document.getElementById("use_meanrev").checked,
    use_reversal:document.getElementById("use_reversal").checked,
    auto_scan:document.getElementById("auto_scan").checked,
    scan_top_n:parseInt(document.getElementById("scan_top_n").value,10),
  };
  const k=document.getElementById("api_key").value.trim();
  const s=document.getElementById("api_secret").value.trim();
  if(k)b.api_key=k;
  if(s)b.api_secret=s;
  await post("/api/cfg",b);
  alert("Saved.");
  cfgFilled=false;   // re-pull from server next cycle
  refresh();
}
...
document.querySelectorAll(".tab").forEach(t=>{t.onclick=()=>{
  ...
  ["dash","cfg","jr"].forEach(id=>{
    document.getElementById("tab-"+id).classList.toggle("hidden",id!==t.dataset.tab);
  });
  if(t.dataset.tab==="jr")loadJournal();
}});

async function loadJournal(){try{
  const rows=await get("/api/journal");
  const el=document.getElementById("journal");
  if(!rows.length){el.textContent="no trades yet";return}
  el.innerHTML=rows.reverse().map(r=>{
    const pnlTxt = (r.pnl!=null) ? r.pnl.toFixed(4) : "";
    const cls = (r.pnl||0)>=0?"pos":"neg";
    const sym = r.symbol || "";
    return `<div class=row><span>${new Date(r.ts).toLocaleTimeString()} <b>${r.event}</b> ${r.side||""} ${sym}</span><span class="${cls}">${pnlTxt}</span></div>`;
  }).join("");
...
def api_state():
    with LOCK:
        debug = list(BOT.state.get("debug", []))
        log = list(BOT.log[-60:])
    return jsonify({
        "running": BOT.running,
        "equity": BOT.state["equity"],
        "position": BOT.state["position"],
        "loops": BOT.state["loops"],
        "last_sig": BOT.state["last_sig"],
        "scanning": BOT.state["scanning"],
        "debug": debug,
...
def api_start():
    BOT.start()
    return jsonify({"ok": BOT.running, "error": BOT.last_error})
...
def api_cfg():
    new = request.json or {}
    cfg = load_cfg()
    allowed = ("api_key", "api_secret", "testnet", "symbol", "timeframe",
               "leverage", "risk_per_trade_pct", "max_daily_loss_pct",
               "max_drawdown_pct", "poll_seconds", "min_signal_score",
               "use_smc", "use_meanrev", "use_reversal",
               "auto_scan", "scan_top_n")
    for k in allowed:
...
if __name__ == "__main__":
    ...
    print(" In Ports tab, set port 8000 to PUBLIC, then open that URL")
    print("=" * 60)
